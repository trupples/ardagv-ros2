...
public:
  ...
  ADI3DToFADTF31xx() : Node("adi_3dtof_adtf31xx_node")
  {
    ...
    param_encoding_type_descriptor.description =
      "Frame encoding type, allowed values: mono16, 16UC1";
    this->declare_parameter<std::string>(
...
private:
   ...
   * @note: Experimental: transform point cloud to laserScan and publish it. Usable in sla
   */
  ...
  void publishImageAndCameraInfo(ADI3DToFADTF31xxOutputInfo * out_frame)
  {
    ...
    //m_disp_image_depth.convertTo(m_depth_8, CV_8UC1, 0.1, 0);
    //cv::applyColorMap(m_depth_8, m_color_depth, cv::COLORMAP_JET);
