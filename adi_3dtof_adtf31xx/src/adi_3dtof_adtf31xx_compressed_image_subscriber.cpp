...
public:
  ...
  bool publishFrames()
  {
    ...
    if (all_callbacks_recvd) {
      ...
      cv::Mat m_depth_image, m_ab_image, m_depth_image_color;
      ...
      cv::applyColorMap(m_depth_image, m_depth_image_color, cv::COLORMAP_JET);
      RCLCPP_INFO(
        this->get_logger(),"%d", m_depth_image.type());
      RCLCPP_INFO(
        this->get_logger(),"%d", m_depth_image_color.type());




      publishImageAsRosMsg(m_depth_image_color, "rgb8", "raw_depth_image", depth_image_publisher_);
