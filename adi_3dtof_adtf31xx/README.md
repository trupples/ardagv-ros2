...
8.  The Node supports sending the Depth and AB images as compressed streams using RVL compression. To publish the compressed images set the ```arg_enable_depth_ab_compression``` parameter to ```1``` in the launch file.  
...
+ **/ab_image**
    - 16-bit AB image of size 512X512
...
+ **/ab_image/compressedDepth** 
    - 512X512 16-bit AB image from sensor node compressed with RVL compression 
...
+ **param_enable_depth_ab_compression** (int, default: 0)
...
+ **param_camera_mode** (String, default: "qmp")
    - Frame Type  
...
3. Clone aditof SDK
    ```bash
    $ cd ~/ros2_ws/src
    $ git clone https://github.com/analogdevicesinc/libaditof.git -b v6.0.0
    ```
...
5. Build the package
    ```bash
    $ cd ~/ros2_ws
    $ colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release -DNXP=1 -DWITH_GLOG_DEPENDENCY=ON -DWITH_NETWORK=ON -DWITH_PROTOBUF_DEPENDENCY=ON --packages-up-to adi_3dtof_adtf31xx
    $ source install/setup.bash
    ```

6. Link runtime libraries
    ```bash
    $ echo "export LD_LIBRARY_PATH=~/ros2_ws/install/lib:\$LD_LIBRARY_PATH" >> ~/.bashrc
    $ source ~/.bashrc
    ```
